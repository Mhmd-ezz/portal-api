...
class DashboardController extends Controller
{
    ...
    function get(Request $request)
    {
        ...
        $monthTo = $request->to;
        $firstDayOfYear = date('Y-01-01',strtotime($from));
        $lastWorkingDay = $this->getLastWorkingDate($today);
        // -----------------------
        // Stats
        // -----------------------
        
        $unassigned = Opportunity::where('rfp_status', '=', 'inprogress')
            ->where(function ($query) use ($user_role) {
                if (!empty($user_role) && $user_role == "executive_manager" || $user_role == "sales_manager" || $user_role == "country_manager")
            ...
            $oppStats = Opportunity::where(function ($query) use ($user_role) {
            if (!empty($user_role) && $user_role == "executive_manager" || $user_role == "sales_manager" || $user_role == "country_manager")
                $query->where('branch_id', Auth::user()->branch_id);
        })
            ...
            ->select(array(
                ...
                DB::raw('IFNULL(SUM(submission_date >= "' . $firstDayOfMonth . '" AND submission_date <= "' . $today . '" AND rfp_status = "inprogress"),0) as overDue'),
