...
class MailHelper
{
    ...
    public static function sendNewTaskAssignedMail(OpportunityTask $task, $send_to_addresses)
    {
        // $opp = Opportunity::find(1);
        // $send_to = ['hasan.ri25@gmail.com'];
        $data = new OpportunityTaskResource($task);
        $data->mailHeader = 'New task created.';
        $priority = 'Low';
        if($data->priority == '1'){
            $priority = 'Normal';
        }else {
        ...
        $dataInput = [
            ...
            'title' => $data->title,
            'priority' => $priority,
            'due_date' => Carbon::parse($data->dueDate)->isoFormat('MMM Do YYYY'),
            'created_at' => Carbon::parse($data->created_at)->isoFormat('MMM Do YYYY'),
        ];
        if (isset($send_to_addresses) && count($send_to_addresses->toArray()) > 0) {
            $send_to_addresses = $send_to_addresses->toArray();
            $send_to = $send_to_addresses;
        ...
        $to_name = "";
        $to_email = $send_to;
        Mail::send("email_task_created_template", $dataInput, function ($message) use ($to_name, $to_email) {
            $message->to($to_email, $to_name)
